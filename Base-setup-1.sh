...
if [[ "$DO_STORAGE" == yes ]]; then
  ...
  if [[ "$STORAGE_MODE" != skip ]]; then
    STORAGE_LV="$(ask "Logical volume name" "lv_patch")"
    [[ "$STORAGE_LV" =~ ^[a-zA-Z0-9._+-]+$ ]] || die "Invalid logical volume name."
    if lvs --noheadings -o lv_name "$STORAGE_VG" 2>/dev/null | grep -qw "$STORAGE_LV"; then
      die "Logical volume $STORAGE_VG/$STORAGE_LV already exists."
    fi
    ...
    # Leaving free extents allows later growth and snapshots
    STORAGE_PCT="$(ask "Percent of free space to allocate (leave headroom for growth)" "95")"
    [[ "$STORAGE_PCT" =~ ^[0-9]{1,3}$ ]] && (( STORAGE_PCT >= 1 && STORAGE_PCT <= 100 )) \
...
if [[ "$STORAGE_MODE" == skip ]]; then
  ...
  FS_SOURCE="/dev/$STORAGE_VG/$STORAGE_LV"
...
if [[ "$STORAGE_MODE" == newvg ]]; then
  ...
  Logical volume:  $STORAGE_LV  at ${STORAGE_PCT}% of free extents
...
rollback_fstab() {
  if [[ -n "$FSTAB_BACKUP" && -f "$FSTAB_BACKUP" && "$DRY_RUN" != yes ]]; then
...
if [[ "$STORAGE_MODE" != skip ]]; then
  ...
  say "Creating logical volume $STORAGE_VG/$STORAGE_LV"
  run lvcreate -l "${STORAGE_PCT}%FREE" -n "$STORAGE_LV" "$STORAGE_VG" \
    || die "lvcreate failed. No filesystem was written."

  LV_PATH="/dev/$STORAGE_VG/$STORAGE_LV"

  say "Creating $STORAGE_FSTYPE filesystem"
  if [[ "$STORAGE_FSTYPE" == xfs ]]; then
    run mkfs.xfs -f "$LV_PATH" || die "mkfs.xfs failed."
  else
    run mkfs.ext4 -F "$LV_PATH" || die "mkfs.ext4 failed."
  fi

  run mkdir -p "$STORAGE_MOUNT" || die "Could not create the mount point."

  if [[ "$DRY_RUN" != yes ]]; then
    FS_UUID="$(blkid -s UUID -o value "$LV_PATH" 2>/dev/null || true)"
    [[ -n "$FS_UUID" ]] || die "Could not read the filesystem UUID."

    FSTAB_BACKUP="/etc/fstab.phase0-$(date +%Y%m%d-%H%M%S).bak"
    cp -a /etc/fstab "$FSTAB_BACKUP"
    log "fstab backed up to $FSTAB_BACKUP"

    if grep -qs "[[:space:]]${STORAGE_MOUNT}[[:space:]]" /etc/fstab; then
      warn "An fstab entry for $STORAGE_MOUNT already exists; leaving it alone."
    else
      printf 'UUID=%s  %s  %s  defaults,noatime  0 0\n' \
        "$FS_UUID" "$STORAGE_MOUNT" "$STORAGE_FSTYPE" >>/etc/fstab
      log "fstab entry added for $STORAGE_MOUNT"
    fi

    systemctl daemon-reload >>"$LOG_FILE" 2>&1 || true

    if ! mount "$STORAGE_MOUNT" >>"$LOG_FILE" 2>&1; then
      rollback_fstab
      die "Mounting $STORAGE_MOUNT failed. fstab was restored; the LV still exists."
    fi
    if ! mount -a >>"$LOG_FILE" 2>&1; then
      rollback_fstab
      die "'mount -a' failed, which would break the next boot. fstab was restored."
    fi
    say "Mounted $STORAGE_MOUNT ($(df -Ph "$STORAGE_MOUNT" | awk 'NR==2{print $2" total, "$4" free"}'))"
  fi
fi
...
CONFEOF
  ...
  REPORT="$STATE_DIR/phase0-report.txt"
  {
    ...
    echo "Storage:   $STORAGE_MODE  $FS_SOURCE ($FS_TYPE) at $FS_TARGET"
...
if [[ "$DRY_RUN" != yes ]]; then
  ...
  if [[ "$STORAGE_MODE" != skip ]]; then
    findmnt -n "$STORAGE_MOUNT" >/dev/null 2>&1 || { warn "$STORAGE_MOUNT is not mounted"; FAILS=$((FAILS+1)); }
  fi
...
printf ' Base:      %s\n' "$PATCH_BASE"
printf ' Storage:   %s\n' "$STORAGE_MODE"
[[ "$STORAGE_MODE" != skip ]] && printf ' Mounted:   %s on %s\n' "$FS_SOURCE" "$FS_TARGET" || true
